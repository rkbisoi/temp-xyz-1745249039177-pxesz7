            ...
            <div className="bg-white rounded-lg shadow-lg border border-gray-200 p-6 mb-8">
                <div className="flex justify-between items-start mb-4">
                    <div className="flex flex-col items-center space-y-2 text-sm text-gray-500">
                        <span className={`px-3 py-1 rounded-full bg-gray-100 text-sm`}>{project?.type}</span>

                    </div>
                    <div className="flex text-sm text-gray-600">
                        <User className="h-4 w-4 mr-1 mt-0.5" />
                        <span>{project?.createdByUser}</span>
                    </div>
                </div>
                    ...
                    <div className="flex flex-row justify-between space-x-2 text-xs text-gray-500 mb-4">
                        <div className="flex items-center mr-1">
                            ...
                            <span>Created: {project?.createDate}</span>
                        </div>
                        <div className="flex items-center">
                            <Clock className="h-4 w-4 mr-1" />
                            {project?.lastUpdated && <span>Updated: {getTimeAgo(convertGMTToLocal(project?.lastUpdated))}</span>}
                        </div>
                    </div>
